...
module Helium.CodeGeneration.PatternMatchEh(patternToEHCore, patternsToEHCore) where
...
import Helium.CodeGeneration.EHCoreUtils

patternsToEHCore :: [(Id, Pattern)] -> EHCore.CExpr -> EHCore.CExpr
patternsToEHCore nps continue = fst (patternsToCore' nps continue 0)

patternsToCore' :: [(Id, Pattern)] -> EHCore.CExpr -> Int -> (EHCore.CExpr, Int)
...
patternToCore :: (Id, Pattern) -> EHCore.CExpr -> EHCore.CExpr
patternToCore np continue = fst (patternToCore' np continue 0)
    
withNr nr e = (e, nr)

patternToCore' :: (Id, Pattern) -> EHCore.CExpr -> Int -> (EHCore.CExpr, Int)
